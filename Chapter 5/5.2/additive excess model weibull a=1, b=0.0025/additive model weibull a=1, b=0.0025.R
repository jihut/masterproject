...
phi <- 0.0025 # scale parameter 
beta.vec <- c(0.05,0.1,0.5) # covariate effects
start.year <- 2000
end.year <- 2021
n.sim.models <- 500
t.grid <- seq(from=0,to=end.year-start.year,by=0.1)

# Store the estimated parameters in each iteration
...
# Store output of proportional excess hazard tests in each iteration
...
# Different extra quantities

min.diff.time.obs <- rep(0,n.sim.models)
prop.excess.deaths <- rep(0,n.sim.models)

# Store estimated baseline and cumulative baseline 
...
for(i in 1:n.sim.models){
  ...
  matrix.time.pop.sim <- vec.pop.sim(age.sim.vec,rep(start.year,n.sim),gender.sim.vec) # again assume that the change of age and year is on the same at the start of the year for simplicity
  ...
  prop.excess.deaths[i] <- length(which(time.excess.sim<=time.pop.sim & delta.i==1))/sum(delta.i==1) # proportion of excess events 
  ...
  br.max.test.em.default.age[i] <- br.max.test.em.default$table[1,"p"]<0.05 # alpha=0.05. Reject null hypothesis of proportional excess hazard if p-value smaller than 0.05. 
...
# Histogram of the estimated parameters for all the variables 
