  ...
  for(i in 1:length(t.grid)){
    ...
    if(is.integer0(index)==T){ # if no one has arrived
      ...
      if(is.integer0(index.pop)==F){
        ...
        excess.hazard.vec <- excess.hazard.piecewise.func(partition.t.vec,chi.vec,new.x.matrix.pop,beta.vec,new.time.obs.vec.pop) # same with excess hazards
      ...
      diff.cumulative.haz.vec <- (rho-1)*cumulative.excess.hazard.piecewise.func(partition.t.vec,chi.vec,new.x.matrix,beta.vec,at.risk.time.vec) # the continuous part for all patients who arrived up to this time point
