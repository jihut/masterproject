# Final CUSUM for EM-based models using local regression # 
...
### span.value.lambda0 = span value when fitting local regression of the estimated baseline excess hazard outputs
### span.value.Lambda0 = span value when fitting local regression of the estimated cumulative baseline excess hazard outputs
...
cusum_r.t.EM.loess <- function(start.year,age.vec,gender.vec,x.matrix,time.obs.vec,
                                          arrival.vec, delta.i.vec,beta.vec,rho,t.grid,
                                          lambda0,Lambda0,event.time.vec,span.value.lambda0=0.5,span.value.Lambda0=0.5,
  ...
  test.data.baseline.fit <- as.data.frame(cbind(lambda0*365.241,event.time.vec/365.241))
  colnames(test.data.baseline.fit) <- c("baseline.haz.values","time.in.years")
  
  baseline.haz.fit <- loess(baseline.haz.values~time.in.years,span=span.value.lambda0,data=test.data.baseline.fit,control = loess.control(surface = "direct"))
  
  baseline.haz.vec.general.func <- function(t){ # t in years 
    baseline.value <- predict(baseline.haz.fit,t)
    baseline.value
  }
  
  excess.hazard.func.EM <- function(x.matrix,beta.vec){
    excess.covariate.part <- as.numeric(exp(beta.vec %*% t(x.matrix)))
    excess.covariate.part
  }
  
  # Cumulative baseline approximate
  
  test.data.cumulative.baseline.fit <- as.data.frame(cbind(Lambda0,event.time.vec/365.241))
  colnames(test.data.cumulative.baseline.fit) <- c("cumulative.baseline.haz.values","time.in.years")
  
  cumulative.baseline.haz.fit <- loess(cumulative.baseline.haz.values~time.in.years,span=span.value.Lambda0,data=test.data.cumulative.baseline.fit,control = loess.control(surface = "direct"))
  
  cumulative.baseline.haz.vec.general.func <- function(t){ # t in years 
    baseline.value <- predict(cumulative.baseline.haz.fit,t)
    baseline.value
  }
  ...
  for(i in 1:length(t.grid)){
    ...
    if(is.integer0(index)==T){ # if no one has arrived
      ...
      if(anyNA(index.pop.old)==F){
        ...
        n.events[i] <- 0
        r.t <- -sum(diff.cumulative.haz.vec) # if none of the observations who have arrived will experience an event throughout the monitoring period.
        R[i] <- r.t
      }
    }
  }
