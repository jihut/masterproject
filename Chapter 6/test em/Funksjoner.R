...
haz.output <- function(Age,Year,haz.data){
  ...
  step <- which(haz.data$Year==(1950) & haz.data$Age==(42))-which(haz.data$Year==(1950-1) & haz.data$Age==(42-1))
...
one.sim <- function(Age,Year,haz.data){
  ...
  if(u> surv.matrix[1,2]){
    t.1 <- surv.matrix[1,1]
    t.2 <- surv.matrix[2,1]
    t.p <- t.1 + (u-1)*(t.2-t.1)/(surv.matrix[1,2]-1)
    delta.p <- 1
  } else {
    for (j in 1:(n)){
      if(surv.matrix[j,2] >u & u>surv.matrix[j+1,2]){
        ...
        t.p <- t.1 + (u-surv.matrix[j,2])*(t.2-t.1)/(surv.matrix[j+1,2]-surv.matrix[j,2])
        delta.p <- 1
        break
      }
    }
...
weibull.obs.net.fast.ver <- function(lambda,phi,beta.vec,x.matrix,Year,t.vec){ # NOTE! O
  ...
  numerator.function.individual.1 <- function(n){ # numerator for t between 0 and 1 
    force(n)
    ...
    amount <- length(index) # amount of observations with this specific combinations of x-values
  ...
  numerator.function.individual <- function(n,k){
    force(n)
    force(k)
    if(distinct.x.matrix[n,2]==0){
      life.table <- pop.data.male
    ...
    # Population individual survival
    cum.pop.hazard<- function(t){ 
      return(sum(haz.data[1:(k-1),2])+(t-haz.data[k,1])*haz.data[k,2]) ### SOMETHING WRONG HERE
    }
    surv.pop.function <- function(t){
      return(exp(-cum.pop.hazard(t)))
    }
    ...
    surv.overall.function<- function(t){ # vectorize both population and excess to get correct result with test function (analytic)?
      return(vec.surv.pop.function(t)*vec.surv.excess.function(t))
    }
    vec.surv.overall.function <- Vectorize(surv.overall.function)
  ...
  for(i in 2:nrow(decoy.haz.matrix)){
    numerator.func <- list()
    denominator.func <- list()
    for(j in 1:nrow(distinct.x.matrix)){
      numerator.func[[j]] <- numerator.function.individual(j,i)
  ...
  integral.between.time.intervals.func <- function(i){
  ...
  integral.between.time.intervals <- sapply(1:nrow(decoy.haz.matrix),integral.between.time.intervals.func)
  cumulative.integral.between.time.intervals <- cumsum(integral.between.time.intervals)
  ...
  final.observable.net.hazard <- function(t){
    ...
    if(i==t){
      return(cumulative.integral.between.time.intervals[i])
    } else {
