...
max.test <- rep(NA,1000)
prop.excess.deaths <- rep(NA,1000)
max.test.instant <- rep(NA,1000)
prop.excess.deaths.instant <- rep(NA,1000)
set.seed(43)
for(i in 1:1000){
  lambda <- 100
  ...
  time.excess.sim.test <- vec.sim.excess.piecewise.in.or.out(t.vec,chi.vec,x.matrix.test,beta.vec,rho,arrival.sim.vec.test,5) # out-of-control after 5 years
  time.excess.sim.test.instant <- vec.sim.excess.piecewise.proportional(t.vec,chi.vec,x.matrix.test,beta.vec,rho) # out-of-control instantly
  ...
  time.observed.sim.test.instant <- pmin(time.pop.sim.test,time.excess.sim.test.instant,time.censoring.sim.test)
  delta.i.test.instant <- pmax(delta.p.test,as.numeric(time.excess.sim.test.instant<time.pop.sim.test))*as.numeric(time.censoring.sim.test>time.observed.sim.test.instant)
  
  # Final data set
  
  prop.excess.deaths[i] <- sum(time.excess.sim.test<=time.pop.sim.test & delta.i.test==1)/sum(delta.i.test==1)
  prop.excess.deaths.instant[i] <- sum(time.excess.sim.test.instant<=time.pop.sim.test & delta.i.test.instant==1)/sum(delta.i.test.instant==1)
  ...
  R.out.control.fast <- cusum_r.t.piecewise(start.year,age.sim.vec.test,gender.sim.vec.test,x.matrix.test,time.observed.sim.test,arrival.sim.vec.test,delta.i.test,chi.vec,t.vec,beta.vec,rho,t.grid,
                                            pop.data.male,pop.data.female,2020)
  max.test[i] <- max(R.out.control.fast[,1])
  R.out.control.fast.instant <- cusum_r.t.piecewise(start.year,age.sim.vec.test,gender.sim.vec.test,x.matrix.test,time.observed.sim.test.instant,arrival.sim.vec.test,delta.i.test.instant,chi.vec,t.vec,beta.vec,rho,t.grid,
                                            pop.data.male,pop.data.female,2020)
  max.test.instant[i] <- max(R.out.control.fast.instant[,1])
}

exceed.threshold <- max.test>threshold.c
sum(exceed.threshold)
exceed.threshold.instant <- max.test.instant>threshold.c
sum(exceed.threshold.instant)
